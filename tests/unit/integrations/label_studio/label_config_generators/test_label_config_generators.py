...
from zenml.integrations.label_studio.label_config_generators import (
    ...
    generate_semantic_segmentation_polygons_label_config,
)
...
@given(label_list=lists(text(min_size=1), min_size=1))
def test_polygon_semantic_segmentation_label_config_generator(
    label_list: List[str],
):
    (
        label_config,
        label_config_type,
    ) = generate_semantic_segmentation_polygons_label_config(label_list)
    first_label = label_list[0]
    assert f"<Label value='{first_label}' />\n" in label_config
    assert label_config_type == AnnotationTasks.SEMANTIC_SEGMENTATION_POLYGONS
    assert label_config is not None
